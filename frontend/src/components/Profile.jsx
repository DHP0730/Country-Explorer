...
const Profile = ({ onLogout }) => {
  ...
  return (
    <div className="absolute top-12 right-0 bg-neutral-900 text-white shadow-2xl rounded-xl w-72 z-50 p-6 animate-fadeIn">
      {/* Profile circle */}
      <div className="flex justify-center mb-4">
        <div className="bg-indigo-200 text-white w-16 h-16 flex items-center justify-center rounded-full text-2xl font-bold">
          {profileLetter}
        </div>
      </div>
      ...
      <div className="text-center mb-4 space-y-1">
        <p className="text-xl font-extrabold text-white">{username || "Username"}</p>
        <p className="text-sm text-gray-100 break-words">{email || "Email"}</p>
        {role && (
